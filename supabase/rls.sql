...
create policy exam_write on exams for all
  using (is_super() or (tenant_id = my_tenant() and my_role() = 'school_admin'))
  with check (is_super() or (tenant_id = my_tenant() and my_role() = 'school_admin'));
-- a teacher may flip published_at on an existing exam (the Gradebook "Publish
-- marks" button) but not create or delete one
...
-- scoped by class AND subject — teaching_assignments is keyed per class+subject,
-- so checking subject alone would let a teacher mark a subject they teach
-- anywhere in the school, not just the classes they're actually assigned
